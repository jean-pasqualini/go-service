...
var (
	...
	// ErrForbidden occurs when a user tries to do something forbidden to then according to
...
func (u User) Create(ctx context.Context, traceID string, nu NewUser, now time.Time) (Info, error) {
	...
	const q = `
		INSERT INTO users (user_id, name, email, password_hash, roles, date_created, date_updated)
	...
	u.logger.Printf(
		...
		traceID,
		"user.Create",
		database.Log(q, qParams...),
	)

	if _, err = u.db.ExecContext(ctx, q, qParams...); err != nil {
