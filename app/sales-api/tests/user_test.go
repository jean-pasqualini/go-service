...
import (
	...
	"github.com/jean-pasqualini/go-service/business/tests"
	"net/http"
	"net/http/httptest"
	"os"
	"strings"
...
func (ut *UserTests) postUser201(t *testing.T) user.Info {
	...
	t.Log("Given the need to create a new user with the users endpoint.")
	{
		...
		t.Logf("\tTest %d:\tWhen using the declared user value.", testID)
		{
			...
			// Define what we wanted to receive. We will just trust the generated fields like ID and Dates so we copy u.
			exp := got
			exp.Name = "Jean Pasqualini"
			exp.Email = "jpasqualini75@gmail.com"
			exp.Roles = []string{auth.RoleAdmin}

			if diff := cmp.Diff(got, exp); diff != "" {
				t.Fatalf("\t%s\tTest %d:\tShould get the expected result : %v", tests.Failed, testID, diff)
			}
...
func (ut *UserTests) putUser204(t *testing.T, id string) {
	...
	t.Log("Given the need to update a user with the users endpoint.")
	{
		...
		t.Logf("\tTest %d:\tWhen using the modified user value.", testID)
		{
			...
			if ru.Email != "jpasqualini75@gmail.com" {
